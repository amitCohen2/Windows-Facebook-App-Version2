...
namespace BasicFacebookFeatures
{
    public partial class FormFindThingsInCommon : FormObjectCollection<User>
    {
        ...
        {
            new Thread(() => initialListBoxComponent<User>(listBoxFriends, m_AllInCommonThingsWithFriend.CommonFriends)).Start();
            new Thread(() => initialListBoxComponent<Group>(listBoxGroups, m_AllInCommonThingsWithFriend.CommonGroups)).Start();
            new Thread(() => initialListBoxComponent<Event>(listBoxEvents, m_AllInCommonThingsWithFriend.CommoEvents)).Start();
            new Thread(() => initialListBoxComponent<Page>(listBoxPages, m_AllInCommonThingsWithFriend.CommonPages)).Start();
        ...
        private void initialListBoxComponent<T>(ListBox i_ListBoxCollection, List<T> i_CommonCollection)
        {
            ...
            if (i_CommonCollection != null)
            {
                foreach (T item in i_CommonCollection)
                {
                    ...
                    try
                    {
                        name = nameProperty?.GetValue(item) as string ?? m_AllInCommonThingsWithFriend.NoNameForItem<T>();
                    }
                    catch (Exception)
                    {
                        name = m_AllInCommonThingsWithFriend.NoNameForItem<T>();
                    }

                    i_ListBoxCollection.Invoke(new Action(() => i_ListBoxCollection.Items.Add(name)));
                }

                if (i_CommonCollection.Count == 0)
                {
                    i_ListBoxCollection.Invoke(new Action(() =>
                    {
